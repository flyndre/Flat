...
import { CustomMapApi } from '@/types/map/CustomMapAPI';
...
const selectedToolRef = defineModel<google.maps.drawing.OverlayType>(
    'selectedTool',
    {
        default: null,
    }
...
const selectedColorRef = defineModel<string>('selectedColor', {
    default: null,
});
watch(selectedColorRef, (v) => setShapeColor(v));

function getShapeIndex(id: string) {
    return all_overlays.findIndex((o) => o.id === id);
}

function deleteShape(id: string) {
    const index = getShapeIndex(id);
    setSelection(all_overlays[index]?.overlay);
    deleteSelectedShape();
}
function centerShape(id: string) {
    const index = getShapeIndex(id);
    setSelection(all_overlays[index]?.overlay);
...
function panMapToShape(id: string) {
    const index = getShapeIndex(id);
    map.value.fitBounds(getShapeBounds(all_overlays[index]?.overlay));
}
function panMapToShapes(ids: string[]) {
    const indeces = ids.map((id) => getShapeIndex(id));
    map.value.fitBounds(
        getShapeListBounds(all_overlays.filter((_v, i) => indeces.includes(i)))
    );
}
...
const api: CustomMapApi = {
    deleteAllDivisions: deleteAllShapes,
    deleteDivision: (d: Division) => deleteShape(d.id),
    deleteSelectedDivision: deleteSelectedShape,
    divisionSelected: shapeSelected,
    focusDivision: (d: Division) => centerShape(d.id),
    panMapToDivision: (d: Division) => panMapToShape(d.id),
    panMapToDivisions: (ds: Division[]) => panMapToShapes(ds.map((d) => d.id)),
    panMapToPos,
    placesTextSearch: placesService.value?.textSearch,
    setDivisionName: (d: Division, n: string) => setShapeName(d.id, n),
};
</script>
